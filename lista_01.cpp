...
typedef struct registro {
  // tipo int 4 bytes	
  int         conteudo;     
  // Aponta para uma estrutura do MESMO tipo registro,
  // logo possui 4 bytes
  struct registro *proximo; 
  
} celula; // Novo tipo de dado chamado celula 
          // com total de 8 bytes.
...
// -------------------------------------------
void inserirDadoInicio(celula *inicio){
   celula *nova; // Nova célula da lista
  ...
  else
  {
   ...
   if (inicio==NULL)
   ...
   else
    {
   	  ...
   	  // O novo elemento aponta para onde apontava o início,
   	  // mantendo assim a conexão com os outros elementos da Lista
	  nova->proximo = inicio;
	   
	  // O início da Lista é atualizado para o novo elemento,
	  // isto é, o NOVO elemento passa a ser o novo início.
	  inicio  = nova; 
	}   	
  }
  menuPrincipal(inicio);
